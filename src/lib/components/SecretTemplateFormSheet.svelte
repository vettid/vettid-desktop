<script lang="ts">
  // Renders a SecretTemplate's fields with per-hint inputs, plus an
  // alias field and (when the template has groupNamePrompt) a group-
  // name field. On save iterates the template's fields and fires one
  // add_secret per non-blank value, all sharing the same alias so the
  // catalog groups them as a single card.
  import { invoke } from '@tauri-apps/api/core';
  import { modal } from '../actions/modal';
  import type { SecretTemplate, FieldHint } from '../secretTemplates';

  interface Props {
    template: SecretTemplate;
    onClose: () => void;
    onSaved: () => void;
  }
  let { template, onClose, onSaved }: Props = $props();

  let groupName = $state('');
  let alias = $state('');
  let values = $state<Record<string, string>>(
    Object.fromEntries(template.fields.map((f) => [f.name, '']))
  );
  let visible = $state<Record<string, boolean>>({});
  let saving = $state(false);
  let error = $state('');

  function combinedAlias(): string {
    const a = alias.trim();
    const g = groupName.trim();
    if (template.groupNamePrompt) {
      if (g && a) return `${g} · ${a}`;
      return g || a;
    }
    return a;
  }

  function inputType(hint: FieldHint): string {
    switch (hint) {
      case 'EMAIL': return 'email';
      case 'PHONE': return 'tel';
      case 'URL':   return 'url';
      // NUMBER intentionally stays as `text` with inputmode="numeric"
      // below — `type=number` coerces bind:value to JS Number, which
      // truncates 16-digit card numbers (precision loss above
      // 9_007_199_254_740_991) and silently drops user spaces. text +
      // inputmode keeps the digit-only keypad on mobile without
      // touching the string value.
      case 'DATE':  return 'date';
      case 'EXPIRY_DATE': return 'month';
      default: return 'text';
    }
  }

  function inputMode(hint: FieldHint): 'numeric' | undefined {
    return hint === 'NUMBER' ? 'numeric' : undefined;
  }

  async function save() {
    if (template.groupNamePrompt && !groupName.trim()) {
      error = `${template.groupNamePrompt} is required.`;
      return;
    }
    const filled = template.fields.filter((f) => (values[f.name] ?? '').trim());
    if (filled.length === 0) {
      error = 'Enter a value for at least one field.';
      return;
    }
    saving = true;
    error = '';
    const finalAlias = combinedAlias() || null;
    try {
      // The vault has no batch op, so each field is its own add. The
      // user approves each on their phone — sequential keeps the order
      // matching the template (which becomes the catalog sort).
      for (const f of filled) {
        const resp: any = await invoke('add_secret', {
          name: f.name,
          value: (values[f.name] ?? '').trim(),
          category: template.category,
          alias: finalAlias,
          description: null,
        });
        if (!resp?.success) {
          error = resp?.error || `Failed to save ${f.name}`;
          saving = false;
          return;
        }
      }
      onSaved();
    } catch (e) {
      error = `Failed to save: ${e}`;
    } finally {
      saving = false;
    }
  }

  function toggleVisible(name: string) {
    visible = { ...visible, [name]: !visible[name] };
  }
</script>

<div class="backdrop" onclick={onClose} role="presentation"></div>
<div class="sheet" role="dialog" aria-modal="true" aria-labelledby="sec-form-title" use:modal={{ onEscape: onClose }}>
  <header class="sheet-head">
    <span class="sheet-ic">{template.icon}</span>
    <h2 id="sec-form-title">{template.name}</h2>
    <button class="close" onclick={onClose} aria-label="Close">✕</button>
  </header>
  {#if error}<div class="error">{error}</div>{/if}

  {#if template.groupNamePrompt}
    <label class="fl">
      <span>{template.groupNamePrompt}</span>
      <input type="text" bind:value={groupName} placeholder={template.groupNamePrompt} />
    </label>
  {/if}
  <label class="fl">
    <span>Alias (optional)</span>
    <input type="text" bind:value={alias} placeholder="e.g. Visa, Amex, Trading Wallet" />
    <span class="h">Helps tell similar entries apart in your catalog.</span>
  </label>

  {#each template.fields as f (f.name)}
    <label class="fl">
      <span>{f.name}</span>
      {#if f.hint === 'NOTE'}
        <textarea bind:value={values[f.name]} rows="2"></textarea>
      {:else if f.hint === 'PASSWORD' || f.hint === 'PIN'}
        <div class="row">
          {#if visible[f.name]}
            <input type="text" inputmode={f.hint === 'PIN' ? 'numeric' : undefined} bind:value={values[f.name]} />
          {:else}
            <input type="password" inputmode={f.hint === 'PIN' ? 'numeric' : undefined} bind:value={values[f.name]} />
          {/if}
          <button type="button" class="rt" onclick={() => toggleVisible(f.name)}>
            {visible[f.name] ? 'Hide' : 'Show'}
          </button>
        </div>
      {:else}
        <input
          type={inputType(f.hint)}
          inputmode={inputMode(f.hint)}
          bind:value={values[f.name]}
        />
      {/if}
    </label>
  {/each}

  <div class="actions">
    <button class="btn ghost" onclick={onClose} disabled={saving}>Cancel</button>
    <button class="btn primary" onclick={save} disabled={saving}>
      {saving ? 'Saving…' : 'Save'}
    </button>
  </div>
  {#if saving}<p class="h" style="margin-top:8px;">Each field is phone-approved — accept on your phone as prompts arrive.</p>{/if}
</div>

<style>
  .backdrop { position: fixed; inset: 0; background: rgba(0,0,0,0.5); z-index: 50; }
  .sheet {
    position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%);
    background: var(--bg-card); border: 1px solid var(--border); border-radius: 12px;
    width: min(440px, 94vw); max-height: 86vh; overflow-y: auto;
    z-index: 51; box-shadow: 0 10px 40px rgba(0,0,0,0.5);
    padding: 18px 20px 20px;
  }
  .sheet-head { display: flex; align-items: center; gap: 10px; margin-bottom: 14px; }
  .sheet-head h2 { margin: 0; font-size: 1.05rem; flex: 1; }
  .sheet-ic { font-size: 1.5rem; }
  .close {
    background: none; border: 1px solid var(--border); border-radius: 4px;
    padding: 3px 9px; cursor: pointer; color: var(--text-muted); font: inherit;
  }
  .close:hover { color: var(--text); background: var(--bg-elevated); }

  .error {
    background: rgba(244,67,54,0.1);
    border: 1px solid rgba(244,67,54,0.25);
    color: var(--error);
    padding: 10px 12px;
    border-radius: 6px;
    margin-bottom: 12px;
    font-size: 0.85rem;
  }

  .fl { display: flex; flex-direction: column; gap: 4px; margin-bottom: 11px; }
  .fl > span:first-child { font-size: 0.78rem; color: var(--text-muted); }
  .fl input, .fl select, .fl textarea {
    padding: 7px 10px;
    background: var(--bg-elevated);
    color: var(--text);
    border: 1px solid var(--border);
    border-radius: 5px;
    font: inherit;
    font-size: 0.9rem;
  }
  .fl textarea { resize: vertical; min-height: 50px; font-family: inherit; }
  .fl input:focus, .fl select:focus, .fl textarea:focus {
    outline: none; border-color: var(--accent);
  }
  .h { font-size: 0.78rem; color: var(--text-muted); }

  .row { display: flex; gap: 6px; }
  .row input { flex: 1; }
  .rt {
    background: rgba(255,255,255,0.08);
    border: 1px solid var(--border);
    color: var(--text-muted);
    border-radius: 5px;
    padding: 6px 10px;
    cursor: pointer;
    font: inherit;
    font-size: 0.78rem;
  }
  .rt:hover { color: var(--text); }

  .actions {
    display: flex; justify-content: flex-end; gap: 8px;
    margin-top: 10px;
  }
  .btn {
    padding: 7px 16px;
    border-radius: 6px;
    cursor: pointer;
    font: inherit;
    font-size: 0.9rem;
    border: 1px solid transparent;
  }
  .btn.ghost { background: transparent; border-color: var(--border); color: var(--text); }
  .btn.ghost:hover { background: var(--bg-elevated); }
  .btn.primary { background: var(--accent); color: #1a1a1a; font-weight: 500; }
  .btn.primary:hover:not(:disabled) { background: var(--accent-hover); }
  .btn:disabled { opacity: 0.5; cursor: not-allowed; }
</style>
